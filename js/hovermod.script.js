...
var getLowestPriceHandler = function (item, pref, callback) {

    var sItem = item;
    // from Steam's community market website
    var appID = sItem.appid;
...
$J(function () {
    ...
    countryCode = typeof (g_rgWalletInfo) != 'undefined' ? g_rgWalletInfo['wallet_country'] : 'US';
    ...
    BuildHover = function (prefix, item, owner) {
        ...
        var el = $J('div[data-economy-item="' + item.appid + '/' + item.contextid + '/' + item.id + '/' + item.owner + '"]');
        if (el.length == 1) {
            el = el[0];
            el.rgItem = item;
        }
            ...
            if (item.fraudwarnings || (g_bIsInventoryPage && item.is_currency)) {
                ...
                if (item.fraudwarnings) {
                    for (var i = 0; i < item.fraudwarnings.length; i++) {
                        ...
                        var warningImage = new Element('img', { 'class': 'fraud_warning_image', src: window.location.protocol + '//cdn.steamcommunity.com/public/images/sharedfiles/icons/icon_warning.png' });
