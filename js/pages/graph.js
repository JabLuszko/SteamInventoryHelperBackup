...
var drawChart = function (data, subtext) {
    ...
    if (drawdays > 0) {
        var firstdate = data[data.length - 1].fulldate;

        for (var i = data.length - 1; i >= 0 ; i--) {
            var item = data[i];

            if (i == 0 || ((firstdate - item.fulldate) / 86400000 >= drawdays)) {
                startidx = i;
                break;
            }
        }
        ...
        for (var i = data.length - 1; i >= 0 ; i--) {
            var item = data[i];
            if (i == 0 || new Date(item.fulldate).getFullYear() != thisyear) {
                startidx = i + 1;
                break;
            }
        }
        ...
        tooltip: {
            ...
            formatter: function () {
                ...
                for (var i = 0; i < points.length; i++) {
                    ...
                    if (point.series.name === 'Bought') {
                        total -= point.y;
                        totalcount += point.options.count;
                    }
                    else if (point.series.name === 'Sold') {
                        total += point.y;
                        totalcount += point.options.count;
                    }
                }
...
var ShowTable = function () {
    chrome.storage.local.get({
        ...
        var strRes = 'Total scanned: ' + (items.lastIdx) + '/' + items.totalRows + ' - Sold: ' + items.totalminus + ' - Bought: ' + items.totalplus + ' - Profit: ' + (Math.round((items.totalminus - items.totalplus) * 100) / 100);
        var ngraph = items.graphdata;
        strRes += '<table border="1" class="table" ><thead><tr><th>Date</th><th colspan="2">Sold</th><th>Acc. sold</th><th colspan="2">Bought</th><th>Acc. bought</th><th>Profit</th><th>Acc. profit</th></tr></thead>';
        var accm = 0, accp = 0;
        ...
        for (var i = 0; i < ngraph.length; i++) {
            ...
            strRes += '<tr><td>' + ngraph[i].date + '</td><td class="align-right">' + (ngraph[i].totalm / 100) + '</td><td class="align-right">' + ngraph[i].countm + '</td><td class="align-right">' + (accm / 100)
                + '</td><td class="align-right">' + (ngraph[i].totalp / 100) + '</td><td>' + ngraph[i].countp + '</td><td class="align-right">' + (accp / 100)
                + '</td><td class="align-right">' + ((ngraph[i].totalm - ngraph[i].totalp) / 100).toFixed(2) + '</td><td class="align-right">' + ((accm - accp) / 100).toFixed(2)
                + '</td></tr>';
        }
        strRes += '</table>';
        $('#spanGraphData').html(strRes);
...
var ProcessPriceData = function (res) {
    if (!res.total_count) {
        ...
        numOfRowGet = 10;
        timer = setTimeout(function () { StartGettingHistory(); }, tmdelay * 2);
        return;
    }
    var m;
    ...
    chrome.storage.local.get({
        ...
        var totalMinus = 0, totalPlus = 0, graphData = []; items.graphdata;
        ...
        var ngraph = null;
        if (items.graphdata.length == 0) ngraph = graphData;
        else {
        ...
        if (numOfRowGet < 100) numOfRowGet += 10;
        if (res.start > 0)
            timer = setTimeout(function () { StartGettingHistory(); }, tmdelay);
    });
}
